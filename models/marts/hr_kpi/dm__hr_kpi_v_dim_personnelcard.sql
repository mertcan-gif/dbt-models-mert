{{
  config(
    materialized = 'view',tags = ['hr_kpi','personnelcard']
    )
}}
...
SELECT    
	...
	,second_manager = CONCAT(act.second_namager_name,' ',act.second_manager_surname)
	,hr_representative_sf_id=act.[hr_responsible_sf_id] 
	...
	,ticket = ''
	,collar_type= CASE WHEN sac.[user_id]='47063484' THEN N'BLUE COLLAR' ELSE N'WHITE COLLAR' END
	,employment_type= CASE WHEN sac.[user_id]='47063484' THEN N'SUBCONTRACTOR' ELSE N'RÖNESANS' END
FROM {{ source('stg_hr_kpi', 'raw__hr_kpi_t_fact_sacreport') }} sac
	...
	LEFT JOIN {{ ref('stg__hr_kpi_v_sf_rmore_actper') }} act ON act.user_id = sac.sap_id
