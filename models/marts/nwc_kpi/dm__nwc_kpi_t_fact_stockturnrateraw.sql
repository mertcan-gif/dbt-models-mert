...
{{
  config(
    materialized = 'table',tags = ['nwc_kpi_draft','stockaging_draft','stockturnoverraw_draft']
    )
}}
	...
	select 
		...
		,right(matnr,8) AS material_code
    ...
    ,cost = case when tcurx.currdec = 3 and (blart = 'WA' or blart = 'WL') and (gkont like '740%' or gkont like '621%') then cast(hsl as decimal(18,2))/10 
