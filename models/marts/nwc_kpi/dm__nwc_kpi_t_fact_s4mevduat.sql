...
SELECT
	[rls_region] = (select top 1 RegionCode from {{ source('stg_dimensions', 'raw__dwh_t_dim_companymapping') }} kuc WHERE mbt.RBUKRS = kuc.RobiKisaKod )
	,[rls_group] = 
		CONCAT(
			COALESCE((select top 1 KyribaGrup from {{ source('stg_dimensions', 'raw__dwh_t_dim_companymapping') }} kuc WHERE mbt.RBUKRS = kuc.RobiKisaKod ),'')
			,'_'
			,COALESCE((select top 1 RegionCode from {{ source('stg_dimensions', 'raw__dwh_t_dim_companymapping') }} kuc WHERE mbt.RBUKRS = kuc.RobiKisaKod ),'')
			)
	,[rls_company] =
		CONCAT(
			COALESCE(RBUKRS,'')
	...
	,[rls_businessarea] = CONCAT('_',(select top 1 RegionCode from {{ source('stg_dimensions', 'raw__dwh_t_dim_companymapping') }} kuc WHERE mbt.RBUKRS = kuc.RobiKisaKod ))
	,KyribaGrup = (select top 1 KyribaGrup from {{ source('stg_dimensions', 'raw__dwh_t_dim_companymapping') }} kuc WHERE mbt.RBUKRS = kuc.RobiKisaKod )
	,KyribaKisaKod = (select top 1 KyribaKisaKod from {{ source('stg_dimensions', 'raw__dwh_t_dim_companymapping') }} kuc WHERE mbt.RBUKRS = kuc.RobiKisaKod )
	,* 
FROM (
	
	SELECT 
		*
		,deposit_demand_group = CASE WHEN LEFT(RACCT,3) = '118' THEN 'Fund' 
									 WHEN LEFT(RACCT,3) IN ('111','112') THEN 'Bond'
									 WHEN HESAP_TIPI_TANIMI IN ('Vadeli','Teminat Vadeli','KKM') THEN 'Deposit' 
								ELSE 'Demand' END
	FROM {{ ref('stg__nwc_kpi_t_fact_s4mevduat') }} mbt
    WHERE 1=1
	AND (RACCT like '102%'  OR RACCT like '111%'  OR RACCT like '118%' OR RACCT like '112%') 
	AND TARIH >= '2023-01-01 00:00:00.000'
