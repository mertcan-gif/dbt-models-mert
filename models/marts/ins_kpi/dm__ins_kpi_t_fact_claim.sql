...
SELECT
	...
	,[rls_businessarea]= CONCAT (business_area,'_',(SELECT TOP 1 dimCompany.rls_region 
									...
									WHERE dimCompany.KyribaKisaKod =  dmg.company)
								)
	...
	,dmg.[branch]
	,main_branch = 
		(SELECT TOP 1 brc.main_branch 
				FROM {{ source('stg_ins_kpi', 'stg__ins_kpi_t_dim_branch') }} brc 
				WHERE brc.branch = dmg.branch)
	,branch_eng = 
		(SELECT TOP 1 brc.branch_eng 
				FROM {{ source('stg_ins_kpi', 'stg__ins_kpi_t_dim_branch') }} brc
				WHERE brc.branch = dmg.branch)
	,main_branch_eng = 
		(SELECT TOP 1 brc.main_branch_eng
				FROM {{ source('stg_ins_kpi', 'stg__ins_kpi_t_dim_branch') }} brc
				WHERE brc.branch = dmg.branch)
	,dmg.[claim_date]
	,dmg.[notice_date]
	...
	,dmg.[cause_of_damage]
	,dmg.[explanation]
	,dmg.[exceptional_damage]
	,dmg.[amount_paid]
	,[currency] = 
				CASE 
					WHEN [currency] = 'EURO' THEN 'EUR'
					WHEN [currency] = 'TL' THEN 'TRY'
					WHEN [currency] = 'POUND' THEN 'GBP'
					WHEN [currency] = 'DZN' THEN 'DZD'
					ELSE [currency]
				END 
	,dmg.[payment_date]
	,dmg.[damage_file_status]
	,dmg.[insurance_department_description] 



	,[exceptional_damage_in_eur] = 		
		CASE
			WHEN dmg.[currency] = 'EURO' THEN exceptional_damage
			WHEN dmg.[currency] = 'EUR' THEN exceptional_damage
			WHEN dmg.[currency] = 'USD' THEN CAST(exceptional_damage / (SELECT TOP 1 b.Value1 FROM BloombergCurrenciesUSD b WHERE b.Currency = 'EUR' AND b.[Date] <= dmg.[claim_date] ORDER BY b.[Date] DESC) AS DECIMAL(18,2))
			WHEN dmg.[currency] = 'TRY' THEN 
									CAST( 
										exceptional_damage / (
													(SELECT TOP 1 b.Value1
														...
														WHERE 1=1 
															AND b.Currency = 'TRY' 
															AND b.[Date] <= dmg.[claim_date] ORDER BY b.[Date] DESC)
													/
	...
	,[amount_paid_in_eur] =
		CASE
			WHEN currency = 'EUR' THEN amount_paid
			WHEN currency = 'USD' THEN CAST(amount_paid / (SELECT TOP 1 b.Value1 FROM BloombergCurrenciesUSD b WHERE b.Currency = 'EUR' AND b.[Date] <= dmg.[claim_date] ORDER BY b.[Date] DESC) AS DECIMAL(18,2))
...
FROM {{ source('stg_ins_kpi', 'stg__ins_kpi_t_fact_claim') }}  dmg 
	LEFT JOIN {{ ref('dm__dimensions_t_dim_companies') }} dimCompanies ON dmg.company = dimCompanies.KyribaKisaKod
WHERE 1=1 
	AND dmg.[claim_date] >= '2020-01-01'
