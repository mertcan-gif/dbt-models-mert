...
SELECT 
	*
	,CASE
		WHEN BLDAT_CPUDT_DIFF > 0 THEN 1
		ELSE 0
...
SELECT
	...
	,late_record_count
	,total_late_volume
	,CAST(AVG(ISNULL(BLDAT_CPUDT_DIFF, 0)) AS DECIMAL(10,2)) average_record_day
	,COALESCE(CAST(AVG(CASE WHEN late_record_flag = 1 THEN ISNULL(BLDAT_CPUDT_DIFF, 0) END) AS DECIMAL(10,2)) ,0) average_late_record_day
	,CAST(AVG(ISNULL(late_record_ratio, 0)) AS DECIMAL(10,2)) late_record_ratio
	,CAST(AVG(ISNULL(late_volume_ratio, 0)) AS DECIMAL(10,2)) late_transaction_volume_ratio
	,COALESCE(CAST(SUM(ISNULL(weighted_average_based_on_late_records, 0)) AS DECIMAL(10,2)) ,0) weighted_average_based_on_late_record
	,COALESCE(CAST(SUM(ISNULL(weighted_average_based_on_total_transaction_volume, 0)) AS DECIMAL(10,2)) ,0) weighted_average_based_on_total_transaction_volume
FROM final f
