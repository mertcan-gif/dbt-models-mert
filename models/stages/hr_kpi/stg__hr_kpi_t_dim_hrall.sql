{{
  config(
    materialized = 'table',tags = ['hr_kpi','hrall']
    )
}}
WITH hr_beyaz_yaka AS (
	...
	SELECT 
		...
		LI.calisan_tipi
	FROM {{ ref('stg__hr_kpi_v_dim_languageindependent') }} LI 
		LEFT JOIN {{ ref('stg__hr_kpi_v_dim_languagedependent') }} LD  on LI.sf_id_number = LD.sf_id_number
)


SELECT hr.* 
	   ,photo_base64 = CASE 
	   						WHEN LEN(pp.photo) < '28000' THEN pp.photo
							ELSE NULL
						END
FROM hr_beyaz_yaka hr
	LEFT JOIN {{ source('stg_hr_kpi', 'raw__hr_kpi_t_dim_profilepictures') }} pp ON pp.userid = hr.employee_id
WHERE 1=1
