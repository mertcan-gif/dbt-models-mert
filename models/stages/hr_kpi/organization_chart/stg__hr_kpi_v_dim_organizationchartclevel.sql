    ...
    materialized = 'view',tags = ['hr_kpi']
		...
		SELECT 
			...
			,CASE 
				...
				ELSE cast(level_entity.[entity_code] as nvarchar(max))
			END	 AS NesneKodu
			,cast(org.pozisyon_kodu as nvarchar(max)) as [pozisyon_kodu]
			,[Bağlı olduğu nesne (Türü)] = CASE WHEN [pstn_entity_parent_code] IS NULL THEN NULL ELSE  'Şirket' END
			,[Bağlı olduğu nesne (Adı)] = [pstn_entity_parent_name]
			,pl.manager_position
			,[Bağlı olduğu nesne kodu] = CAST(CAST([pstn_entity_parent_code] AS INT) AS NVARCHAR(MAX))
			,[Organizasyon Şeması Bağlı olduğu nesne (Türü)] =
				CASE 
					WHEN level_entity.[org_entity_samelevel_code] IS NOT NULL THEN 'Bölge/Fonksiyon/BU'
					WHEN level_entity.head_of_unit_position IS NOT NULL THEN 'Şirket'
					--WHEN level_entity.
					ELSE NULL
				END	  
		...
		FROM {{source('stg_hr_kpi', 'raw__hr_kpi_t_dim_personnellist') }} pl
		    LEFT JOIN {{ source('stg_hr_kpi', 'raw__hr_kpi_t_dim_division')}} level_entity ON pl.pozisyon_kodu = level_entity.head_of_unit_position
