  ...
  config(
    materialized = 'view',tags = ['hr_kpi']
		...
		SELECT 
			...
			,[Bağlı olduğu nesne (Türü)] = CASE WHEN [pstn_entity_parent_code] IS NULL THEN NULL ELSE  'Bölge/fonksiyon/BU' END
			...
			,[Organizasyon Şeması Bağlı olduğu nesne (Türü)] =
				CASE 
					WHEN level_entity.[org_entity_samelevel_code] IS NOT NULL THEN 'Bölüm/Proje/İşletme'
					WHEN level_entity.head_of_unit_position IS NOT NULL THEN 'Bölge/fonksiyon/BU'
					--WHEN level_entity.
					ELSE NULL
				END	  
		...
		FROM  {{ source('stg_hr_kpi', 'raw__hr_kpi_t_dim_personnellist') }} pl
			LEFT JOIN {{ source('stg_hr_kpi', 'raw__hr_kpi_t_dim_subdivision') }} level_entity ON pl.pozisyon_kodu = level_entity.head_of_unit_position
