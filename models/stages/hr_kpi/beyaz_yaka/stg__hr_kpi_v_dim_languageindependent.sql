{{
  config(
    materialized = 'view',tags = ['hr_kpi','hr_li']
    )
}}
...
SELECT 
	...
	,rls_company = UPPER(sac.is_birimi_en)
	...
	,dwh_date_of_recruitment = CASE
									WHEN sac.[son_ronesans_baslangic_tarihi_revize] IS NOT NULL THEN CONVERT(DATETIME,sac.[son_ronesans_baslangic_tarihi_revize], 104)
	...
	,ronesans_rank = CASE 
						 when sac.[grup/baskanlik_en] = 'PUSULA' THEN '6'
...
WHERE 1=1
	...
	AND sac.[grup/baskanlik_en] <> 'STAJYER'
	AND sac.[grup/baskanlik_en] <> 'OTHER'
	AND sac.[grup/baskanlik_en] <> 'DESNA ESTATE INVESTMENT HOLDING'
	AND 1 = CASE
...
SELECT
	...
	,dwh_cause_details
	,dwh_ronesans_last_seniority = CASE
										WHEN [employee_status] = 'A' THEN DATEDIFF(DAY, CONVERT(DATETIME, [dwh_date_of_recruitment], 104), GETDATE())
										ELSE DATEDIFF(DAY, CONVERT(DATETIME, [dwh_date_of_recruitment], 104), CONVERT(DATETIME, [dwh_date_of_termination], 104)) 
									END
