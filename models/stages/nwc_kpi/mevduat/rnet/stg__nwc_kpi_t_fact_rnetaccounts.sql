...
WITH Tally AS (
    ...
    SELECT TOP (DATEDIFF(DAY, (SELECT MIN([start_date]) FROM {{ ref('stg__nwc_kpi_t_fact_depositcockpit_rnet') }}), 
                          (SELECT MAX(end_date) FROM {{ ref('stg__nwc_kpi_t_fact_depositcockpit_rnet') }})) + 1)
	...
	SELECT 
		...
		t_base.end_date,
		t_base.account_type
	FROM {{ ref('stg__nwc_kpi_t_fact_depositcockpit_rnet') }} t_base
	CROSS APPLY (
