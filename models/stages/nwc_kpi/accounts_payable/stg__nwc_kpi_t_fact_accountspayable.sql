...
(
	SELECT 
		...
		**/
		...
		,company_code = ''
		,company_name = ''
		,in_out_group_type = CASE WHEN LEN(LFA1.LIFNR) = 3 THEN 'In Group' WHEN LEN(LFA1.LIFNR) <> 3 THEN 'Out Group' END
		--,[SOURCE] = 'SAP'
	...
	UNION ALL
		...
		SELECT
			...
			**/
			...
			CPUDT = ''  ,
			MAINACCOUNT = '' , 
			KUNNR = CAST(s.SupplierId AS nvarchar), -- Müşteri Kodu
			CUSTOMER = CAST(s.SupplierId AS nvarchar)   , -- Müşteri Adı
			...
			WSL = AmountCurrency - AmountSettledCurrency,
			HSL = ''  ,
			DUEDATE = CAST(DueDate AS DATE)  ,
			DUE_DAYS = DATEDIFF(DAY,CAST(GETDATE() AS DATE),CAST(DueDate AS DATE))  
			/**
