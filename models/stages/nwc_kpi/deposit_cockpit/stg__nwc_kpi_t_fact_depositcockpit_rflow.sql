...
WITH cte_rnet AS (
	...
	SELECT 
		company = sirketkodu --BUKRS
		,company_description = sirkettanim --BUTXT
		,bank_country = ulkekodu --LAND1
		,bank_code = bankkod --KOD
		,bank_name = b.cmbBankCode_TEXT --TANIM
		,[control] = '01' --AS KNTRL
		,account_type = CASE 
			WHEN hesaptipi = '1' THEN '02'
			WHEN hesaptipi = '2' THEN '01'
		 END  --HSTIP
		,amount_transaction_currency = pbrm  --RTCUR
		,txt_balance = cast(tutar as money)--DMBE3
		,txt_rate = oran
		,txt_maturity_start_date = CASE WHEN bastarih = '0000-00-00' THEN NULL ELSE CAST(bastarih AS DATE) END
		,txt_due_date = CASE WHEN bitistarih = '0000-00-00' THEN NULL ELSE CAST(bitistarih AS DATE) END
		,account_key = concat(tutanak_no,tutartarih) --CONCAT(FORMID,ORDERID,lstCompanyDesc,lstPB,cmbBankCode,cmbAccountType,cast(txtBalanceDate as date))
		,account_key_without_date = tutanak_no --CONCAT(FORMID,ORDERID,lstCompanyDesc,lstPB,cmbBankCode,cmbAccountType)
		,balance_date = CASE WHEN tutartarih = '0000-00-00' THEN NULL ELSE CAST(tutartarih AS DATE) END
		,snapshot_date = CAST(snapshot_date as date)
	from {{ source('stg_fi_kpi', 'raw__fi_kpi_t_fact_rflowbankbalances') }} rf
		left join (select distinct cmbBankCode, cmbBankCode_TEXT from "aws_stage"."fi_kpi"."raw__nwc_kpi_t_fact_rnetbankbalances") b
			on b.cmbBankCode = rf.bankkod COLLATE Latin1_General_CI_AS
)
